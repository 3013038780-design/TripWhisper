...
import { generate, initialProfile, adjust } from '../lib/journey.ts';
...
for (const card of explorePlaceCards) {
  ...
  assert.doesNotMatch(card.desc, /实时(营业|班次|票价|余票)/);
}
...
assert.equal(formatFetchedAt(resolveProvenance(userStop).fetchedAt), '尚未记录核验时间');
...
const stamped = preserveUserProvenance(prev, next, '2026-09-12T11:00:00.000Z');
...
const exportedDemo = payload.journey.days[0].stops.find((s) => s.id === 'duomo');
